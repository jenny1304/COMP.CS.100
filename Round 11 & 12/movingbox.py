...
Student Id: 153131981, 153151026
Name:       Huyen Pham, Khoi Ha
Email:      huyen.pham@tuni.fi, khoi.ha@tuni.fi
12.5 Project: Moving Boxes
"""
...
class MovingBox:
    ...
    def __init__(self, name):
        """
        Constructor. Initialize the name of the box with a dictionary items
        """
        self.__name = name
        self.__items = {}
    ...
    def get_items(self):
        """
        Get the dictionary items of the object who called the function
        """
        return self.__items

    def add_item(self, item_name, item_count):
        """
        Add the given amount of named items into a named moving box.
        :param item_name: str, name of item to add
        :param item_count: str, number of items to add
        """
        if item_name not in self.__items.keys():
    ...
    def list_content(self):
        """
        Lists in alphabetical order the items and their amounts stored in the named box.
        """
        sum = 0
        for item_name in self.__items:
            sum += self.__items[item_name]

        print(f'Box "{self.__name}" contains {sum} items.')

        if sum >0:
            for item_name in sorted(self.__items.keys()):
                if self.__items[item_name] != 0:
                    print(f"{self.__items[item_name]:3} {item_name}")

    def remove_item(self,item_name,item_count):
        """
        Removes a given amount of a named item from a named moving box.
        :param item_name: str, name of item to remove
        :param item_count: str, number of items to remove
        """
        if item_name not in self.__items.keys():
            return
        if self.__items[item_name] < item_count:
            return
    ...
    def transfer_item(self, target_box, item_name,item_count):
        """
        Moves a given number of named items from one box to another.
        :param item_name: MovingBox, box to transfer item to
        :param item_name: str, name of item to transfer
        :param item_count: str, number of items to transfer
        """
    ...
    def search_item(self, item_name):
        """
        Search for item in the box, if it exists then return the number of that item
        """
